...
describe('Visitor Permissions', () => {
  ...
  it('shows error on blocked contacts component when no contact has been selected', async () => {
    ...
    expect(
      await screen.findByTextKey('registration_settings_blocked_contacts_error_none_selected')
    ).toBeInTheDocument();
    expect(await axe(container)).toHaveNoViolations();
  });

  it('shows error on blocked contacts exclusion list when no contact has been selected', async () => {
    const { container } = render(
      <TestWrapper>
        <MockedProvider mocks={mocks}>
          <AppFeaturesProvider value={appFeatures}>
    ...
    expect(
      await screen.findByTextKey('registration_settings_blocked_contact_groups_error_none_selected')
    ).toBeInTheDocument();
    expect(await axe(container)).toHaveNoViolations();
  });
  ...
  it('shows error on allowed contacts group component when no contact group has been selected', async () => {
    ...
    const editAllowedContactComponent = within(screen.getByTestId('edit-allowed-contactlist-fields'));
    const contactsCheckbox = editAllowedContactComponent.getByRole('checkbox', {
      name: /Select contact groups/i
    });
    fireEvent.click(contactsCheckbox);
    expect(contactsCheckbox).toBeChecked();
    ...
    expect(
      await screen.findByTextKey('registration_settings_allowed_contact_group_error_none_selected')
    ).toBeInTheDocument();
    expect(await axe(container)).toHaveNoViolations();
  });
  ...
  it('shows error on allowed contacts type component when no contact type has been selected', async () => {
    ...
    expect(
      await screen.findByTextKey('registration_settings_allowed_contact_type_error_none_selected')
    ).toBeInTheDocument();
    expect(await axe(container)).toHaveNoViolations();
  });
